...
import { UserPlus, CheckCircle, XCircle, Search, Loader2, AlertCircle, RefreshCw } from 'lucide-react';
...
const ChildRequestsManager: React.FC<ChildRequestsManagerProps> = ({ adminId }) => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      const data = filter === 'ALL'
        ? await childRequestService.getRequestsByStatus()
        : await childRequestService.getRequestsByStatus(filter);
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      const result = await childRequestService.approveRequest(
        requestId,
        adminId,
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      const result = await childRequestService.rejectRequest(
        requestId,
        adminId,
        response
      );
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Child Linking Requests</h2>
                ...
                <div className="space-y-3 pt-4 border-t border-gray-200">
                  <div>
                    <label htmlFor={`response-${request.id}`} className="block text-sm font-medium text-gray-700 mb-2">
                      Admin Response {request.status === 'PENDING' && '(Optional for approval, required for rejection)'}
                    </label>
                    <textarea
